...
// pip.itsonlyasd.com | KV: pip-pad-kv
// For Pip (age 6), 7" display, Pi 5 Ubuntu kiosk
...
async function handleYTSearch(request, env, cors) {
  ...
  const disneyMode = url.searchParams.get('disney') === '1';
  if (disneyMode) {
    const DISNEY_FEATURED_QUERIES = [
      'Walt Disney Pictures logo intro 1985',
      'Disney classic movie opening 1980s vintage',
      'Aladdin 1992 opening scene Disney',
      'Dumbo 1941 opening Disney classic',
      'Sleeping Beauty 1959 intro Disney vintage',
      'Cinderella 1950 opening Disney classic',
      'Snow White 1937 Disney opening',
      'Bambi 1942 opening Disney',
      'Pinocchio 1940 Disney opening',
      'Disney sing along songs classic',
      'Mickey Mouse Clubhouse full episode',
      'Disney classic cartoon compilation'
    ];
    q = DISNEY_FEATURED_QUERIES[Math.floor(Math.random() * DISNEY_FEATURED_QUERIES.length)];
  }
  ...
  try {
    ...
    let results = [];
    // Primary: parse InnerTube JSON (only if it actually returned JSON)
    const ct = res.headers.get('content-type') || '';
    if (ct.includes('json')) {
      const data = await res.json();
      const contents = data?.contents?.twoColumnSearchResultsRenderer?.primaryContents
        ?.sectionListRenderer?.contents?.[0]?.itemSectionRenderer?.contents || [];
      results = renderItems(contents);
    }
    // Fallback: scrape the results page for ytInitialData (robust from a Worker)
    if (!results.length) {
      results = await scrapeSearch(q);
    }
    return new Response(JSON.stringify({ results }), {
      headers: { 'Content-Type': 'application/json', ...cors }
    });
  } catch (e) {
    // Last-ditch fallback so the grid still fills even if InnerTube throws
    try {
...
// Robust fallback: fetch the HTML results page and pull ytInitialData out of it.
async function scrapeSearch(q) {
  ...
  const m = txt.match(/ytInitialData\s*=\s*(\{.+?\});<\/script>/s) || txt.match(/ytInitialData"\]\s*=\s*(\{.+?\});/s);
  if (!m) return [];
...
// Safe KV read: never throws even if the KV binding is missing/undefined.
async function kvGet(env, key) {
  try { if (!env || !env.PIP_KV) return null; return await kvGet(env, key); }
  catch (e) { return null; }
}
async function kvPut(env, key, val) {
  try { if (!env || !env.PIP_KV) return false; await env.PIP_KV.put(key, val); return true; }
  catch (e) { return false; }
}

async function handleGetSettings(request, env, cors) {
  let settings = DEFAULT_SETTINGS;
  try { const raw = await kvGet(env, 'settings'); if (raw) settings = JSON.parse(raw); } catch (e) {}
...
async function handleSaveSettings(request, env, cors) {
  ...
  const pin = body.pin;
  const stored = await kvGet(env, 'pin') || '0000';
  if (pin !== stored) return new Response(JSON.stringify({ error: 'wrong pin' }), { status: 401, headers: { 'Content-Type': 'application/json', ...cors } });
  const current = JSON.parse(await kvGet(env, 'settings') || JSON.stringify(DEFAULT_SETTINGS));
  const updated = { ...current, ...body.settings };
  await kvPut(env, 'settings', JSON.stringify(updated));
...
async function handleSetMode(request, env, cors) {
  ...
  const stored = await kvGet(env, 'pin') || '0000';
  if (pin !== stored) return new Response(JSON.stringify({ error: 'wrong pin' }), { status: 401, headers: { 'Content-Type': 'application/json', ...cors } });
  const settings = JSON.parse(await kvGet(env, 'settings') || JSON.stringify(DEFAULT_SETTINGS));
  settings.parentMode = !!body.parentMode;
  await kvPut(env, 'settings', JSON.stringify(settings));
  return new Response(JSON.stringify({ ok: true }), { headers: { 'Content-Type': 'application/json', ...cors } });
}
...
/* Search panel */
/* Pip's favorite category buttons */
#keyword-row { display:flex; gap:8px; overflow-x:auto; padding-bottom:8px; margin-bottom:8px; scrollbar-width:none; }
#keyword-row::-webkit-scrollbar { display:none; }
.kw-btn { flex-shrink:0; padding:9px 15px; border-radius:22px; border:1px solid #2a2a3e; background:#111824; color:#cfd2e0; font-size:14px; font-weight:600; cursor:pointer; white-space:nowrap; transition:all .15s; }
.kw-btn:active { background:#6c63ff33; color:#fff; border-color:#6c63ff; transform:scale(.95); }
.kw-btn.disney { background:linear-gradient(135deg,#1a1040,#2a1a60); color:#c4a6ff; border-color:#6c63ff55; }
.kw-btn.disney:active { background:linear-gradient(135deg,#6c63ff,#9b8aff); color:#fff; }
...
/* ── KID-FRIENDLY FULLSCREEN RESULTS GRID (Pip) ── */
#results-overlay {
  position:fixed; inset:0; z-index:60; background:#0a0a12;
  display:none; flex-direction:column;
}
#results-overlay.open { display:flex; }
#ro-head {
  display:flex; align-items:center; gap:12px; padding:16px 18px;
  background:#10101c; border-bottom:1px solid #1e1e30; flex-shrink:0;
}
#ro-title { flex:1; font-size:18px; font-weight:700; color:#fff; }
#ro-close {
  width:54px; height:54px; border-radius:16px; border:none;
  background:#ff6b6b22; color:#ff8a8a; font-size:30px; line-height:1; cursor:pointer;
  display:flex; align-items:center; justify-content:center; flex-shrink:0;
}
#ro-close:active { background:#ff6b6b44; }
#ro-grid {
  flex:1; overflow-y:auto; -webkit-overflow-scrolling:touch;
  display:grid; grid-template-columns:repeat(2,1fr); gap:16px; padding:18px;
  align-content:start;
}
#ro-grid::-webkit-scrollbar { width:0; }
.ro-card {
  background:#14142a; border-radius:18px; overflow:hidden; cursor:pointer;
  border:3px solid transparent; transition:transform .12s, border-color .12s;
}
.ro-card:active { transform:scale(.96); border-color:#6c63ff; }
.ro-card img { width:100%; aspect-ratio:16/9; object-fit:cover; display:block; background:#222; }
.ro-card .ro-t { font-size:15px; line-height:1.3; color:#fff; padding:10px 12px 4px; font-weight:600;
  display:-webkit-box; -webkit-line-clamp:2; -webkit-box-orient:vertical; overflow:hidden; }
.ro-card .ro-d { font-size:12px; color:#8a8aa0; padding:0 12px 12px; }
#ro-status { color:#777; font-size:16px; padding:40px; text-align:center; grid-column:1/-1; }
...
<div id="controls">
  ...
  <div class="tab-panel active" id="panel-search">
    <div id="keyword-row">
      <button class="kw-btn disney" onclick="searchDisney()">✨ Disney Classics</button>
      <button class="kw-btn" onclick="searchKeyword('Blippi educational videos for kids','🎪 Blippi')">🎪 Blippi</button>
      <button class="kw-btn" onclick="searchKeyword('Mickey Mouse Clubhouse toys kids','🐭 Mickey')">🐭 Mickey Mouse Toys</button>
      <button class="kw-btn" onclick="searchKeyword('Pez candy dispenser collection kids','🍬 Pez')">🍬 Pez</button>
      <button class="kw-btn" onclick="searchKeyword('marble run satisfying kids','🔵 Marble Runs')">🔵 Marble Runs</button>
      <button class="kw-btn" onclick="searchKeyword('domino falling amazing satisfying kids','🀡 Dominoes')">🀡 Dominoes</button>
    </div>
    <div id="search-row">
...
<div id="results-overlay">
  <div id="ro-head">
    <div id="ro-title">Pick a video</div>
    <button id="ro-close" onclick="closeResults()">✕</button>
  </div>
  <div id="ro-grid"></div>
</div>
  </div>
...
// ── YouTube Search ──
function openResults(){ document.getElementById('results-overlay').classList.add('open'); }
function closeResults(){ document.getElementById('results-overlay').classList.remove('open'); }

async function runGridSearch(apiUrl, label) {
  const grid = document.getElementById('ro-grid');
  document.getElementById('ro-title').textContent = label || 'Pick a video';
  grid.innerHTML = '<div id="ro-status">Looking\u2026</div>';
  openResults();
  try {
    const data = await fetch(API + apiUrl).then(r=>r.json());
    grid.innerHTML = '';
    if (!data.results?.length) {
      grid.innerHTML = '<div id="ro-status">No videos found \u2014 try another word</div>';
      return;
    }
    data.results.forEach(v => {
      ...
      card.innerHTML = \`
        ...
        <div class="ro-t">\${v.title}</div>
        <div class="ro-d">\${v.duration||''}</div>
      \`;
      card.onclick = () => { playYT(v.videoId, v.title); closeResults(); };
      grid.appendChild(card);
    });
  } catch(e) {
    grid.innerHTML = '<div id="ro-status">Couldn\u2019t search right now \u2014 try again</div>';
  }
}
function searchKeyword(query, label) {
  try { document.getElementById('search-input').value = label || query; } catch(e){}
  runGridSearch('/api/yt/search?q=' + encodeURIComponent(query), label || query);
}
function searchDisney() {
  try { document.getElementById('search-input').value = '\u2728 Disney Classics'; } catch(e){}
  runGridSearch('/api/yt/search?disney=1', '\u2728 Disney Classics');
}
...
async function doSearch() {
  ...
  if (!q) return;
  const grid = document.getElementById('ro-grid');
  document.getElementById('ro-title').textContent = q;
  grid.innerHTML = '<div id="ro-status">Looking\u2026</div>';
  openResults();
  try {
